...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded text-[10px] sm:text-[11px] font-sans uppercase tracking-wider transition-all duration-200 border ${
              isOwner
                ? 'bg-emerald-950/60 text-emerald-300 border-emerald-500/50'
                : 'bg-[#121110] text-[#D6B85A] border-[#C9A227]/40 hover:border-[#C9A227] hover:text-[#F3F0E8]'
            }`}
            title="Owner Portal & Management"
          >
            <Lock className="w-3 h-3 text-[#C9A227]" />
            <span className="font-semibold">{isOwner ? 'Portal' : 'Owner'}</span>
          </button>
      ...
      {/* Mobile Drawer Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#0A0A0A] border-b border-[#C9A227]/25 px-4 pt-3 pb-6 space-y-3.5 shadow-2xl animate-in slide-in-from-top duration-200">
          <div className="text-xs text-[#A9A59C] pb-2 border-b border-[#C9A227]/15 flex items-center gap-1.5">
            <MapPin className="w-3.5 h-3.5 text-[#C9A227]" />
            <span className="truncate">243, M.G. Road (Opposite Bina Cinema), Budge Budge</span>
          </div>
          ...
          {/* Dedicated Mobile Owner Panel Section */}
          <div className="pt-2 border-t border-[#C9A227]/25 space-y-2">
            <div className="p-3.5 rounded-2xl bg-gradient-to-br from-[#161410] via-[#0d0c0a] to-[#12110e] border border-[#C9A227]/40 shadow-xl space-y-2.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-[#C9A227]/20 border border-[#C9A227]/50 flex items-center justify-center text-[#D6B85A]">
                    <Lock className="w-3.5 h-3.5 text-[#C9A227]" />
                  </div>
                  <div>
                    <div className="text-[11px] font-sans font-bold uppercase tracking-wider text-[#F3F0E8] flex items-center gap-1.5">
                      Owner Management Section
                    </div>
                    <div className="text-[9px] text-[#A9A59C]">
                      Full Boutique Control for Mobile
                    </div>
                  </div>
                </div>
                <div
                  className={`px-2 py-0.5 rounded-full text-[9px] font-mono flex items-center gap-1 border ${
                    isOwner
              ...
              {/* Primary Owner Access Button */}
              <button
                id="mobile-drawer-owner-main-btn"
                onClick={() => {
                  setIsMobileMenuOpen(false);
                  onOpenAdmin('photos');
                }}
                className="w-full py-2.5 px-3.5 rounded-xl gold-gradient-bg text-black font-semibold text-xs flex items-center justify-between shadow-lg shadow-[#C9A227]/20 active:scale-98 transition-all"
              >
                <div className="flex items-center gap-2">
                  <Lock className="w-3.5 h-3.5 text-black" />
                  <span>{isOwner ? 'Launch Owner Dashboard' : 'Access Owner Panel (Login)'}</span>
                </div>
                <ChevronRight className="w-4 h-4 text-black" />
              </button>

              {/* Mobile Direct Fast-Action Grid */}
              <div className="grid grid-cols-2 gap-1.5 pt-0.5">
                <button
                  ...
                  className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-left flex items-center gap-2 text-stone-300 active:scale-95 transition-all"
                >
                  <Camera className="w-3.5 h-3.5 text-[#C9A227] shrink-0" />
                  <div className="truncate">
                    <div className="text-[10px] font-semibold text-white">Photos Hub</div>
                    <div className="text-[8px] text-stone-400">Cover &amp; Slides</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="truncate">
                    <div className="text-[10px] font-semibold text-white">Catalog</div>
                    <div className="text-[8px] text-stone-400">Products &amp; Stock</div>
                  </div>
                </button>
                ...
                <button
                  onClick={() => {
                    setIsMobileMenuOpen(false);
                    onOpenAdmin('add-product');
                  }}
                  className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-left flex items-center gap-2 text-stone-300 active:scale-95 transition-all"
                >
                  <Camera className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                  <div className="truncate">
                    <div className="text-[10px] font-semibold text-white">+ Add Item</div>
                    <div className="text-[8px] text-stone-400">Phone Camera</div>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="truncate">
                    <div className="text-[10px] font-semibold text-white">Offers</div>
                    <div className="text-[8px] text-stone-400">Festive Deals</div>
                  </div>
                </button>
              </div>
            </div>
          ...
          {/* Customer Concierge Phone Button */}
          <div className="pt-1 flex gap-2">
            <a
              href={`tel:${storeInfo.phone}`}
              className="w-full py-2.5 rounded-xl bg-[#121110] text-[#D6B85A] border border-[#C9A227]/40 hover:bg-[#C9A227]/15 text-xs font-semibold flex items-center justify-center gap-2 uppercase tracking-wider font-sans"
            >
              <Phone className="w-3.5 h-3.5 text-[#C9A227]" />
              Call Showroom ({storeInfo.phone})
            </a>
          </div>
        </div>
      )}
